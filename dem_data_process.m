...
for task = [1, 2]
    switch task
        ...
        print(gcf, ['/mnt/PRESKY/user/weishuo/DEM_Download/map/Model-elv.png'], '-r200', '-dpng')
        clf

        contourf(elv_1');hcb = colorbar; caxis([0 5500])
        set(hcb, 'ytick', [0:500:5500], 'yticklabel', 0:500:5500)
        title('Model-elv-mask', 'fontsize', 12)
        print(gcf, ['/mnt/PRESKY/user/weishuo/DEM_Download/map/Model-elv-mask.png'], '-r200', '-dpng')
        clf

        contourf(dem_ERA5);hcb = colorbar; caxis([0 5500])
        set(hcb, 'ytick', [0:500:5500], 'yticklabel', 0:500:5500)
        title('DEM-elv', 'fontsize', 12)
        print(gcf, ['/mnt/PRESKY/user/weishuo/DEM_Download/map/DEM-elv.png'], '-r200', '-dpng')
        clf
